...
% This data was used by Bland and Altman 1999 and 2007 from:
% [L.S. Bowling, W.S. Sageman, S.M. O’Connor, R. Cole, and D.E. Amundson.
% 1993. Lack of agreement between measurement of ejection fraction by
% impedance cardiography versus radionuclide ventriculography. Critical
% Care Medicine 21: 1523–1527.]
% In BA1999 the table (Table 4) differs from the table in BA2007 (Table 1).
% The corresponding figure in BA2007 (Figure 1) does not display the same
...
% table in BA2007 is incorrect and the table from BA1999 is correct. Thus,
% codata.mat contains the BA1999 cardiac output data.
% Unfortunately, the original data source (Bowling et al. 1993) doesn't
% tabulate the data in the article. Bland and Altman probably got it
...
% This section is implemented in ba.m based on the 2007 article by Bland
% and Altman ‘Agreement between methods of measurement with multiple
% observations per individual’ (BA2007), because the section in BA1999
% contains an error. The 2007 article corrects this error and provides a
% calculation example with the same data as the BA1999 article. The numbers
% calculated below are the numbers from BA2007 §3 (‘Method where the true
