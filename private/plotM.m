...
sM = scatter(x,y);
sM.ZData = 1:n;
UserData = dcStruct([],'µ',sMYName,'i',[],@dcXYZ);
if any(m>1)
...
if doPlotRegStats
    ...
    if doConReg
        ...
        strLoaDC = sprintf('%.2f×s_res',z);
    else
        strLoaFun = sprintf('\\sqrt{\\pi/2}f_{res}(\\mu)');
        strLoaDC = sprintf('%.2f×f_res(µ)',z*sqrt(pi/2));
    end
    regLineLLoa.UserData = dcStruct( ...
        sprintf( ...
        'f_lower(µ) - %.2f×f_res(µ) = %s×µ + %s', ...
        sqrt(pi/2)*z, ...
...
elseif doMSD
    ...
    strSubObs = sprintf('\\itn\\bf = %u subjects, \\it\\Sigmam\\bf = %u observation pairs',n,N);
end
if doMSD
    xlabel('subject mean')
    ylabel(sMYLongName)
    ...
    xlabel(sprintf('mean \\itµ = (M_1+M_2)/2'))
